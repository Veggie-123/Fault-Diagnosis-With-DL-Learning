...
class TimeFreqAttentionModel(nn.Module):
    ...
    2. 双向GRU序列建模
    3. 注意力机制
    4. 三路特征融合（注意力输出 + 原始GRU输出 + CNN投影输出）
    5. 全局平均池化
    6. 全连接层 + Dropout + Softmax
    """
    def __init__(self, input_channels, num_classes=10,
        ...
        # 第二个注意力层：作用于三路融合后的特征
        self.attention2 = AttentionLayer(gru_output_dim)
    ...
    def forward(self, x):
        ...
        # 对三路融合后的特征再次应用注意力机制
        # 让模型学习如何进一步加权融合后的特征
        attn_out2, attn2 = self.attention2(residual_out)  # (B, T, 2H) 加权序列, (B, T) 注意力权重
        
        # 使用注意力权重的加权和（而不是全局平均池化）
        # attn2是 (B, T)，attn_out2是 (B, T, 2H)
        # 对时间维度做加权求和：(B, T, 2H) × (B, T, 1) -> (B, 2H)
        context = torch.sum(attn_out2 * attn2.unsqueeze(-1), dim=1)  # (B, 2H)

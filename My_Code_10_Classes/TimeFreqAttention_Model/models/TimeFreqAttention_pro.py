...
class TimeFreqAttentionModel(nn.Module):
    ...
    def __init__(self, input_channels, num_classes=10,
        ...
        # 因为CN, C_out, L)，而融合时需要与 (B, T, 2H) 相加
    ...
    def forward(self, x):
        ...
        residual_out = gru_out + cnn_out  # (B, T, 2H) 2路融合
        
        # 注意力机制：输出加权后的序列（保持时间维度）
        attn_out, attn = self.attention(residual_out)     # (B, T, 2H) 加权序列, (B, T) 注意力权重

        context = torch.sum(attn_out, dim=1)
